...
export function buildSingleSessionReviewRequest(
  ...
  return [
    ...
    "- The prompt contains the complete relevant diff and policy. Review them directly without repository tools.",
    "",
...
export function buildLensPrompt(params: {
  ...
  return [
    ...
    ``,
    `Assigned-lens calibration:`,
    `- Stay inside the assigned lens. Do not repeat work owned by another review lane.`,
    `- Inspect related code only when it is needed to prove or reject a candidate within this lens.`,
    `- Report only medium-to-high risk issues with verified impact.`,
...
export function buildSynthesisPrompt(params: {
  ...
  return [
    ...
    ``,
    `Ponytail lens:`,
    `- Apply Ponytail while validating candidate findings. Do not start a third full audit.`,
    `- Use candidate reports as the work queue. Inspect only the code needed to prove or reject each candidate.`,
    ...
    `- Do not simplify away security, validation, error handling, or tests that prevent real regressions.`,
    `- Surface a Ponytail finding only when a candidate identifies concrete maintainability, correctness, or operational risk in changed code.`,
    ``,
